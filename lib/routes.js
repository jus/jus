...
module.exports = function(server) {
  const filename = 'app-server-404.html'
  ...
  server.get('/favicon.ico', (req, res, next) => {
    var faviconPath = path.join(server.sourceDir, 'favicon.ico')
  ...
  server.respondWithFile = (href, req, res) => {
    ...
    if(file.isRenderable) {
      ...
      return file.render(context, function(err, output){
        // Avoid silent errors. Plugin authors need users feedback to fix bugs
        if (err) throw err
