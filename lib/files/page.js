...
module.exports = class Page extends File {
  ...
  // It's redundant work for every page to figure out the base URL...
  // TODO: Move this out of the Page class and do it once, elsewhere.
  setAbsoluteUrl() {
    var pkg = path.join(this.sourceDir, 'package.json')
    if (!exists(pkg)) return
    var homepage = require(pkg).homepage || require(pkg).website
    if (!homepage) return
    // URL.parse()
    // this.absoluteUrl = 
  }
  ...
  addTwitterCardMetaTags($) {
    if ($('head').length && this.title && this.description) {
      $('head').append(`
  ...
  render(context, done) {
    ...
    this.addTitleTagToHead($)
    this.addTwitterCardMetaTags($)
