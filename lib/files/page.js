...
module.exports = class Page extends File {
  ...
  parseMarkdownOrHTML() {
    ...
    // Hang on to the DOM object in case want to query it elsewhere
    this.$ = $
  }
  ...
  setTitle() {
    ...
    // Add <title> tag to page, if absent
    /*
      Disabled for now because it shouldn't be injected into a page until
      after a layout has been applied.. that way there's a <head> to inject
      <title> into...
    */

    // if (!$('title').length && $('*').length) {
    //   $('*').first().before(`<title>${this.title}</title>`)
    // }
  }

  // Stingify the cleaned up DOM tree
  domToHTML() {
    this.content.processed = this.$.html()
  }
