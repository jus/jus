...
module.exports = function associate(pages, images, dataFiles) {
  ...
  Object.keys(pages).forEach(function (href) {
    ...
    page.content.processed = render(page.content.processed)({page: page, pages: pages})

    // Render into page template
    if (pages['/page']) {
      var pageTemplate = pages['/page'].content.original
      page.content.processed = render(pageTemplate)({page: page})
    }

    // Render into layout template
    if (pages['/layout']) {
      var layoutTemplate = pages['/layout'].content.original
      page.content.processed = render(layoutTemplate)({body: page.content.processed})
    }
